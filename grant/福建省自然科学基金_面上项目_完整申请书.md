...
在分子层面，睡眠-觉醒状态转换伴随脑内大规模转录重编程[4,5]。早期利用 bulk 组织 RNA-seq 已发现睡眠剥夺改变数百个基因的表达，涉及即刻早期基因（IEGs）、昼夜节律基因、代谢通路和免疫应答等[6,7]。但 bulk 测序掩盖了细胞类型特异性信号：不同细胞类型（神经元、星形胶质细胞、小胶质细胞等）对睡眠剥夺的响应截然不同甚至相反[8]。近年来，单细胞 RNA 测序（scRNA-seq）技术的发展使得在单细胞分辨率解析复杂脑组织的转录异质性成为可能[9]。2024年，Bruning等首次将 scRNA-seq 应用于睡眠剥夺小鼠模型，揭示了前额叶皮层各细胞类型的差异响应[10]，但该研究仅覆盖皮层单个脑区，下丘脑——睡眠-觉醒调控的核心枢纽——的全细胞转录组响应仍未被系统探索。2024年，Guo等利用单核RNA-seq对小鼠视前区下丘脑进行分析（158,960个核），发现仅一种甘丙肽能神经元亚型在睡眠剥夺恢复期被特异性激活[16]，但其研究聚焦于睡眠恢复而非急性剥夺，且未涉及Pomc神经元及下游神经肽加工通路。You等（2024）对GSE137665数据集（即本项目所分析的数据）的独立再分析确认了睡眠剥夺在GABA能神经元中引发显著的氧化应激和整合应激反应通路激活[17]。然而，这些研究仍未触及从单个差异基因到功能性GRN级联反应的系统解析。从单个差异基因到功能调控环路的跨越，是理解睡眠剥夺从分子扰动到生理表型的必经之路。
...
Pomc 主要在下丘脑弓状核（ARC）的 Pomc 神经元中高表达，也少量表达于脑干孤束核和垂体前叶。Pomc 神经元是公认的"厌食神经元"（anorexigenic neurons），其激活可强烈抑制摄食[12]。近年来研究提示 Pomc 神经元活性受睡眠-觉醒状态调节：Goldstein等（2018）发现 Pomc 神经元在觉醒期活性升高，在睡眠期降低；且人为激活下丘脑 Pomc 神经元可促进觉醒[13]。在转录调控层面，Pomc 启动子含有保守的 CREB、STAT3 和 AP-1 家族转录因子结合位点，这些 TF 是整合代谢、应激和昼夜节律信号的关键节点——值得注意的是，CREB、AP-1（c-Fos/JunB）和 Nr4a 家族正是我们 GRN 分析中与 Pomc 共表达最强的转录因子，形成了文献与计算的独立交叉验证。然而，睡眠缺失对 Pomc 基因本身转录水平的影响、Pomc 蛋白加工通路的下游效应、以及 Pomc 在睡眠剥夺转录组中的网络调控地位，此前均未被系统研究。
...
**(5) Pcsk2启动子转录因子结合位点预测**

我们进一步对Pcsk2启动子区（−2000 ~ +100 bp，参考mm39）进行了JASPAR 2024数据库的TF结合位点扫描，重点关注GRN中与Pomc共表达的15个TFs。结果显示：CREB1（105个预测位点，最优位于−291 bp）、c-Fos（69个位点，−607 bp为经典TRE基序ATGAGTCA，评分=1.00）、JunB（216个位点，−607 bp反向链评分0.99）和EGR1（4个位点，最优−144 bp）在Pcsk2启动子区均有高置信度结合位点。−607 TRE同时被c-Fos和JunB识别，形成AP-1复合物的经典靶序列；−291位点同时被CREB1和ATF3识别，提示为CRE/ATF响应元件。这些计算预测为内容三中ChIP-qPCR实验的候选靶区域提供了明确的优先验证位点。
...
16. Guo X, Keenan BT, Sarantopoulou D, et al. Single-nucleus RNA-seq identifies one galanin neuronal subtype in mouse preoptic hypothalamus activated during recovery from sleep deprivation. *Cell Rep*. 2024;43(5):114192. doi:10.1016/j.celrep.2024.114192
17. You J, et al. Exploring the potential relationship between short sleep risks and cognitive function from the perspective of inflammatory biomarkers and cellular pathways: Insights from population-based and mice studies. *CNS Neurosci Ther*. 2024;30(5):e14783. doi:10.1111/cns.14783
